      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl overflow-hidden transition-colors duration-300">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="animate-slide-up-delay">
            <InfoCard
              label="Serial Number"
              value={formatSerialDisplay(deviceInfo.serialNumber)}
              icon={
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a1.994 1.994 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                </svg>
              }
              onCopy={() => copyToClipboard(deviceInfo.serialNumber, 'serial')}
...
const InfoCard: React.FC<InfoCardProps> = ({ label, value, icon, onCopy, isCopied, highlight = false }) => {
  return (
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center mb-2">
            <span className={`${highlight ? 'text-primary-600 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400'} mr-2 transition-colors duration-300`}>
              {icon}
            </span>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 transition-colors duration-300">{label}</label>
          </div>
          <p className={`font-mono text-lg transition-colors duration-300 ${highlight ? 'text-primary-800 dark:text-primary-200 font-semibold' : 'text-gray-900 dark:text-gray-100'}`}>
            {value}
          </p>
        </div>
